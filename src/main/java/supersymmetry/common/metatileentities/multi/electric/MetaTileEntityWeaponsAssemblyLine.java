...
import static supersymmetry.common.blocks.BlockSheetedFrame.SHEETED_FRAME_AXIS;

public class MetaTileEntityWeaponsAssemblyLine extends RecipeMapMultiblockController {
    ...
    public MetaTileEntityWeaponsAssemblyLine(ResourceLocation metaTileEntityId) {
        super(metaTileEntityId, SuSyRecipeMaps.WEAPONS_ASSEMBLY_LINE_RECIPES);
    }

    @Override
    public MetaTileEntity createMetaTileEntity(IGregTechTileEntity tileEntity) {
        return new MetaTileEntityWeaponsAssemblyLine(metaTileEntityId);
    }
    ...
    protected BlockPattern createStructurePattern() {
        FactoryBlockPattern pattern = FactoryBlockPattern.start(FRONT, UP, RIGHT)
                .aisle("FIF", "RTR", "SAC", " Y ")
                .aisle("FIF", "RTR", "DAG", " Y ").setRepeatable(3, 11)
                .aisle("FOF", "RTR", "CAC", " Y ")
                ...
                .where('G', states(getSheetedState()))
                .where('A',
                        states(SuSyBlocks.MULTIBLOCK_CASING.getState(BlockSuSyMultiblockCasing.CasingType.WEAPON_ASSEMBLY_CONTROL)))
                .where('R', states(MetaBlocks.TRANSPARENT_CASING.getState(BlockGlassCasing.CasingType.TEMPERED_GLASS)))
                .where('T',
                        states(SuSyBlocks.MULTIBLOCK_CASING.getState(BlockSuSyMultiblockCasing.CasingType.WEAPON_ASSEMBLY_LINE_CASING)))
    ...
    @NotNull
    protected static IBlockState getSheetedState() {
        return SuSyMetaBlocks.SHEETED_FRAMES.get(Materials.StainlessSteel).getDefaultState().withProperty(SuSyMetaBlocks.SHEETED_FRAMES.get(Materials.StainlessSteel).variantProperty, Materials.StainlessSteel).withProperty(SHEETED_FRAME_AXIS, BlockSheetedFrame.FrameEnumAxis.X);
    }
    ...
    protected static TraceabilityPredicate dataHatchPredicate() {
        ...
        if (ConfigHolder.machines.enableResearch) {
            return abilities(MultiblockAbility.DATA_ACCESS_HATCH, MultiblockAbility.OPTICAL_DATA_RECEPTION)
                    .setMaxGlobalLimited(1, 0)
                    .or(states(getSheetedState()));
        }
        return states(getSheetedState());
    }
